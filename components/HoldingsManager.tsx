...
const HoldingsManager: React.FC = () => {
  ...
  // Chart Data
  const chartData = visibleHoldings
    .map((h) => ({
      name: h.name,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 md:col-span-2 shadow-sm">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <div
                className={`text-4xl font-extrabold mt-1 flex items-center gap-2 ${getColor(
                  totalMyProfit
                )}`}
              >
                {totalMyProfit >= 0 ? (
          ...
          {showAddForm && (
            <form
              ...
              className="mt-6 p-4 bg-slate-900 rounded-lg border border-slate-700 grid grid-cols-2 md:grid-cols-5 gap-3 animate-fade-in-down"
            >
              <div className="col-span-1">
                ...
                <input
                  placeholder="AAPL"
                  className="w-full bg-slate-800 text-white p-2 rounded border border-slate-600 focus:border-blue-500 outline-none text-sm"
              ...
              <div className="col-span-1">
                ...
                <input
                  ...
                  placeholder="0"
                  className="w-full bg-slate-800 text-white p-2 rounded border border-slate-600 focus:border-blue-500 outline-none text-sm"
                  value={newStock.profit}
                  onChange={(e) =>
                    setNewStock({ ...newStock, profit: Number(e.target.value) })
                  }
                />
              </div>
              <button
                type="submit"
                className="col-span-2 md:col-span-1 mt-auto bg-blue-600 hover:bg-blue-500 text-white rounded p-2 font-bold text-sm transition"
              >
                保存
              </button>
            </form>
        ...
        <div className="hidden md:flex bg-slate-800 p-4 rounded-xl border border-slate-700 flex-col items-center justify-center">
          <h4 className="text-slate-400 text-xs uppercase mb-2 font-bold">
            资产分布
          </h4>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-700">
              {visibleHoldings.map((holding) => {
                // 由于不再有allUsers数据，我们简化显示，只显示当前用户的持仓标记
                const isMyHolding = holding.userId === currentUser?.id;
                return (
                  <tr
                    key={holding.id}
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-slate-300">
                          {isMyHolding ? "我的持仓" : "他人持仓"}
                        </span>
                      </div>
                    </td>
                    <td
                      className={`p-4 text-right font-mono font-bold ${getColor(
                        holding.profit
                      )}`}
                    >
                      {holding.profit > 0 ? "+" : ""}
                      ...
                      {holding.userId === currentUser?.id ? (
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onBlur={(e) =>
                              handleUpdateNote(holding.id, e.target.value)
                            }
                          />
                          <Edit2
                            size={12}
        ...
        <div className="md:hidden divide-y divide-slate-700">
          {visibleHoldings.map((holding) => {
            ...
            return (
              <div
                key={holding.id}
                className="p-4 hover:bg-slate-750 transition"
              >
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    <span className="text-xs text-slate-500 bg-slate-900 px-1.5 py-0.5 rounded font-mono">
                      {holding.symbol || "N/A"}
                    </span>
                  </div>
                  <div
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2 text-xs text-slate-400">
                    ...
                    <span>{isMyHolding ? "我的持仓" : "他人持仓"}</span>
                  </div>
                  {isMyHolding && (
                    <button
                      onClick={() => removeHolding(holding.id)}
